...
it("a custom keyboard channel leaves primary keyboard fallback active", () => {
  const chartStore = createChartStore<number>(
    { type: "line" },
    (value) => value,
    (value) => value,
  ...
  render(
    <SensorManager
      sensors={[KeyboardSensor({ name: "custom-keyboard" })]}
      value={value}
    />,
  );
  engine.input({
  ...
  expect(access.getInteraction("primary-hover")?.target?.data).toBe(1);
