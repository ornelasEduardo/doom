...
describe('Drawer Component', () => {
  it('does not render when closed', () => {
    render(<Drawer isOpen={false} onClose={() => {}} title="Test Drawer">Content</Drawer>);
    expect(screen.queryByText('Test Drawer')).not.toBeVisible();
  });

  it('renders when open', () => {
    render(<Drawer isOpen={true} onClose={() => {}} title="Open Drawer">Content</Drawer>);
    expect(screen.getByText('Open Drawer')).toBeInTheDocument();
  });

  it('calls onClose when clicking overlay', () => {
    const handleClose = vi.fn();
    render(<Drawer isOpen={true} onClose={handleClose} title="Test">Content</Drawer>);
